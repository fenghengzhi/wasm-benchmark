use wasm_bindgen::prelude::*;
#[cfg(target_arch = "wasm32")]
use std::arch::wasm32::*;

#[wasm_bindgen]
pub fn render_mandelbrot(width: u32, height: u32, max_iter: u32) -> Vec<u8> {
    let mut pixels = vec![0u8; (width * height * 4) as usize];
    let x_min = -2.5_f64;
    let x_max =  1.0_f64;
    let y_min = -1.2_f64;
    let y_max =  1.2_f64;

    for py in 0..height {
        for px in 0..width {
            let cx = x_min + px as f64 * (x_max - x_min) / width as f64;
            let cy = y_min + py as f64 * (y_max - y_min) / height as f64;
            let mut zx = 0.0_f64;
            let mut zy = 0.0_f64;
            let mut iter = 0u32;
            while zx * zx + zy * zy <= 4.0 && iter < max_iter {
                let tmp = zx * zx - zy * zy + cx;
                zy = 2.0 * zx * zy + cy;
                zx = tmp;
                iter += 1;
            }
            let idx = ((py * width + px) * 4) as usize;
            if iter < max_iter {
                let t = iter as f64 / max_iter as f64;
                pixels[idx]     = (9.0  * (1.0-t) * t*t*t * 255.0) as u8;
                pixels[idx + 1] = (15.0 * (1.0-t)*(1.0-t) * t*t * 255.0) as u8;
                pixels[idx + 2] = (8.5  * (1.0-t)*(1.0-t)*(1.0-t) * t * 255.0) as u8;
            }
            pixels[idx + 3] = 255;
        }
    }
    pixels
}

// ── SHA-256 ───────────────────────────────────────────────────────

const K: [u32; 64] = [
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
];

#[wasm_bindgen]
pub fn sha256(data: &[u8]) -> Vec<u8> {
    let mut h0: u32 = 0x6a09e667;
    let mut h1: u32 = 0xbb67ae85;
    let mut h2: u32 = 0x3c6ef372;
    let mut h3: u32 = 0xa54ff53a;
    let mut h4: u32 = 0x510e527f;
    let mut h5: u32 = 0x9b05688c;
    let mut h6: u32 = 0x1f83d9ab;
    let mut h7: u32 = 0x5be0cd19;

    // Pre-processing: 直接处理原始数据的完整块，只为尾部构造 padding
    let data_len = data.len();
    let bit_len = (data_len as u64) * 8;
    let full_blocks = data_len / 64;
    let tail_start = full_blocks * 64;
    let tail_need = data_len % 64;
    let pad_len = if tail_need < 56 { 64 } else { 128 };
    let mut tail = vec![0u8; pad_len];
    tail[..tail_need].copy_from_slice(&data[tail_start..]);
    tail[tail_need] = 0x80;
    tail[pad_len - 8..].copy_from_slice(&bit_len.to_be_bytes());

    // Process each 512-bit (64-byte) block
    let mut w = [0u32; 64];
    let all_chunks = data[..tail_start].chunks_exact(64).chain(tail.chunks_exact(64));
    for chunk in all_chunks {
        // Build message schedule
        for i in 0..16 {
            w[i] = ((chunk[i * 4] as u32) << 24)
                 | ((chunk[i * 4 + 1] as u32) << 16)
                 | ((chunk[i * 4 + 2] as u32) << 8)
                 | (chunk[i * 4 + 3] as u32);
        }
        for i in 16..64 {
            let s0 = w[i - 15].rotate_right(7) ^ w[i - 15].rotate_right(18) ^ (w[i - 15] >> 3);
            let s1 = w[i - 2].rotate_right(17) ^ w[i - 2].rotate_right(19) ^ (w[i - 2] >> 10);
            w[i] = w[i - 16].wrapping_add(s0).wrapping_add(w[i - 7]).wrapping_add(s1);
        }

        let mut a = h0;
        let mut b = h1;
        let mut c = h2;
        let mut d = h3;
        let mut e = h4;
        let mut f = h5;
        let mut g = h6;
        let mut h = h7;

        for i in 0..64 {
            let s1 = e.rotate_right(6) ^ e.rotate_right(11) ^ e.rotate_right(25);
            let ch = (e & f) ^ ((!e) & g);
            let temp1 = h.wrapping_add(s1).wrapping_add(ch).wrapping_add(K[i]).wrapping_add(w[i]);
            let s0 = a.rotate_right(2) ^ a.rotate_right(13) ^ a.rotate_right(22);
            let maj = (a & b) ^ (a & c) ^ (b & c);
            let temp2 = s0.wrapping_add(maj);

            h = g;
            g = f;
            f = e;
            e = d.wrapping_add(temp1);
            d = c;
            c = b;
            b = a;
            a = temp1.wrapping_add(temp2);
        }

        h0 = h0.wrapping_add(a);
        h1 = h1.wrapping_add(b);
        h2 = h2.wrapping_add(c);
        h3 = h3.wrapping_add(d);
        h4 = h4.wrapping_add(e);
        h5 = h5.wrapping_add(f);
        h6 = h6.wrapping_add(g);
        h7 = h7.wrapping_add(h);
    }

    let mut result = Vec::with_capacity(32);
    for v in [h0, h1, h2, h3, h4, h5, h6, h7] {
        result.extend_from_slice(&v.to_be_bytes());
    }
    result
}

// ── Int32 Matrix Multiply (SIMD) ──────────────────────────────────

#[wasm_bindgen]
pub fn matmul_i32_simd(a: &[i32], b: &[i32], n: u32) -> Vec<i32> {
    let n = n as usize;

    // Transpose B for sequential memory access
    let mut bt = vec![0i32; n * n];
    for i in 0..n {
        for j in 0..n {
            bt[j * n + i] = b[i * n + j];
        }
    }

    let mut c = vec![0i32; n * n];
    let simd_chunks = n / 4;

    for i in 0..n {
        let ai = i * n;
        for j in 0..n {
            let bj = j * n;
            let mut acc = i32x4_splat(0);

            for k in 0..simd_chunks {
                let off = k * 4;
                unsafe {
                    let va = v128_load(a.as_ptr().add(ai + off) as *const v128);
                    let vb = v128_load(bt.as_ptr().add(bj + off) as *const v128);
                    acc = i32x4_add(acc, i32x4_mul(va, vb));
                }
            }

            let mut sum = i32x4_extract_lane::<0>(acc)
                + i32x4_extract_lane::<1>(acc)
                + i32x4_extract_lane::<2>(acc)
                + i32x4_extract_lane::<3>(acc);

            // Handle remainder
            for k in (simd_chunks * 4)..n {
                sum += a[ai + k] * bt[bj + k];
            }

            c[ai + j] = sum;
        }
    }
    c
}
